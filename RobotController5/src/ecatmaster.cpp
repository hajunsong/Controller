...
void f_task_rt_ecat_task(void *v_local_ptr_cookie)
{
    ...
    while (REF(v_area_ecat_task_run_ptr))			//Task run flag
    {
        ...
        local_slave_data.cur_status_word = v_area_ecat_control->f_get_slave()[0].v_global_cur_status_word;
        local_slave_data.tar_position    = v_area_ecat_control->f_get_slave()[0].v_global_tar_position;
        local_slave_data.tar_velocity    = v_area_ecat_control->f_get_slave()[0].v_global_tar_velocity;
        local_slave_data.tar_torque      = v_area_ecat_control->f_get_slave()[0].v_global_tar_torque;
        local_slave_data.cur_position    = v_area_ecat_control->f_get_slave()[0].v_global_cur_position;
        local_slave_data.cur_velocity    = v_area_ecat_control->f_get_slave()[0].v_global_cur_velocity;
        local_slave_data.cur_torque      = v_area_ecat_control->f_get_slave()[0].v_global_cur_torque_value1;
    ...
    while(1)
    {
        ...
        if(len > 0){
            for(unsigned int i = 0; i < len; i++){
                fprintf(v_area_ecat_control->fp, "%d,%d,%ld,%ld,%ld,%ld,%d,%d\n",
                        v_area_ecat_control->slave_data.front().indx, v_area_ecat_control->slave_data.front().cur_status_word,
                        v_area_ecat_control->slave_data.front().tar_position, v_area_ecat_control->slave_data.front().cur_position,
                        v_area_ecat_control->slave_data.front().tar_velocity, v_area_ecat_control->slave_data.front().cur_velocity,
                        v_area_ecat_control->slave_data.front().tar_torque,   v_area_ecat_control->slave_data.front().cur_torque);
