...
userinterface::userinterface()
{
    ...
    v_global_ethercat_control.f_oper_slave_setting_initialize(0, // slave number
                                                            "DAR Servo", // slave device name
                                                            "DAR", // slave vendor name
...
void userinterface::on_v_action_pb_ECAT_start_clicked()
{
    ...
    if(!REF(v_area_req_ecat_state))
    {
        ...
        v_global_ethercat_control.f_oper_master_initialize(1, // slave total count
                                                           1, // domain total count
                                                           1000000, // free run sycle time
 ...
 *  Return value  : None.
 */
void userinterface::f_oper_stateUpdateTimer_Callback()
{
#if 1
    std::string str;


    if(v_global_recheck_flag == 0){
        for (int i = 0; i<MAX_SLAVES; i++)
        {
            v_global_prev_online_slave_state[i].state.online = 1;
            v_global_recheck_flag = 1;
        }
    }

//        str = v_global_ethercat_control.f_oper_get_ecat_state();
//        str += " , WC " + QString::number(REF(v_global_ethercat_control.f_get_cur_ecat_state_ptr()));
//        ui->v_action_labal_ECAT_state->setText(str);

    str = v_global_ethercat_control.f_oper_get_ecat_state();
    REF(v_global_ethercat_control.f_get_cur_ecat_state_ptr());


#endif


    v_global_cur_all_state = v_global_ethercat_control.f_oper_get_servo_valid_state();

    if(v_global_prev_all_state != v_global_cur_all_state)
    {
        v_global_prev_all_state = v_global_cur_all_state;
        // servo state update
        switch(v_global_cur_all_state)
        {
...
//            ui->v_action_labal_servo_state->setText("Servo On");
            ...
            {
                if(v_global_ethercat_control.f_get_slave()[slv_inx].v_global_req_servo_state == REQ_SERVO_ON)
...
void userinterface::on_v_action_pb_servo_RUN_clicked()
{
    ...
    for(int slv_inx=0; slv_inx < v_global_ethercat_control.f_get_slave_count(); slv_inx++)
    {
       v_global_ethercat_control.f_get_slave()[slv_inx].v_global_run_speed = 5;
