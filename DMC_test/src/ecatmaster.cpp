...
void ecatmaster::f_oper_set_servo_state(int v_local_slv_inx)
{
    ...
    if(v_global_ecat_slave[v_local_slv_inx].v_global_prev_step != v_global_ecat_slave[v_local_slv_inx].v_global_cur_step)
    {
        ...
        case step_servo_on:	// servo on
            ...
            EC_WRITE_U32(v_global_domain1_pd + v_global_ecat_slave[v_local_slv_inx].f_oper_get_offset_value(TARGET_PORSITION)
...
std::string ecatmaster::f_oper_get_ecat_state()
{
...
int ecatmaster::f_oper_slave_setting_initialize(int v_local_slave_num,
                                                std::string v_local_slave_models,
...
void ecatmaster::time_diff_print(int id, int print_mode)
{
    ...
    case 1:
        ...
        if(id == 0){
            ...
            n_tot[id] += abs((int32_t)n_dc[id]-(int32_t)n_gap[id]);
        }
        else{
            n_gap[id] = (ts_gap[id]);
            n_tot[id] += n_gap[id];
...
void f_task_rt_ecat_task(void *v_local_ptr_cookie)
{
    ...
    rt_event_clear(v_area_event_desc_ptr,
               EVENT_WAIT_APP,
               &v_area_mask_ret);
    ...
    while (REF(v_area_ecat_task_run_ptr))			//Task run flag
    {
        ...
        {
            for(int v_sub_count =0;
                v_sub_count < v_area_ecat_control->f_get_slave_count();
                ++v_sub_count)
            {
