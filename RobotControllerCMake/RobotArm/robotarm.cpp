...
void RobotArm::zyx2mat(double rz, double ry, double rx, double *mat)
{
    ...
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < 3; j++){
            for(int k = 0; k < 3; k++){
                R_yaw_R_pitch[i*3+j] += R_z[i*3+k]*R_y[k*3+j];
            }
        }
    }
    ...
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < 3; j++){
            mat[i*3+j] = 0;
            for(int k = 0; k < 3; k++){
                mat[i*3+j] += R_yaw_R_pitch[i*3+k]*R_x[k*3+j];
            }
        }
    }
}
...
RobotArm::RobotArm(uint numbody, uint DOF, double step_size) {
    ...
    if(MODULE_TYPE == DataControl::Module::FAR_V1){
    ...
    else if(MODULE_TYPE == DataControl::Module::FAR_V2 || MODULE_TYPE == DataControl::Module::FAR_V3){
        // DH paramter
        // | Link | alpha(deg) |  a(mm)  |  d(mm)   | theta(deg) |
        // |=====================================================|
        // |  1   |    -90     |  0      |   0      |    -90     |
        ...
        double factor = 1.0;
        body[6].mi = 4.33236347828486e-002*factor;
        body[6].Ixx = 1.6349828551908e-004*factor;
        body[6].Iyy = 1.64408120008878e-004*factor;
        body[6].Izz = 3.19367458327637e-006*factor;
        body[6].Ixy = 6.41534880621834e-008*factor;
        body[6].Iyz = 2.49712406360536e-007*factor;
        body[6].Izx = -8.84019788476247e-006*factor;
        body[6].Jip[0] = body[6].Ixx; body[6].Jip[1] = body[6].Ixy; body[6].Jip[2] = body[6].Izx;
        body[6].Jip[3] = body[6].Ixy; body[6].Jip[4] = body[6].Iyy; body[6].Jip[5] = body[6].Iyz;
        body[6].Jip[6] = body[6].Izx; body[6].Jip[7] = body[6].Iyz; body[6].Jip[8] = body[6].Izz;
        body[6].u_vec[0] = 0; body[6].u_vec[1] = 0; body[6].u_vec[2] = 1;
        body[6].tool_offset[0] = 0.03;  body[6].tool_offset[1] = -0.124;    body[6].tool_offset[2] = -0.005;
...
int RobotArm::run_inverse_kinematics(double* input_q, double* des_pose, double* cur_joint, double* cur_pose){
    ...
    double mat[9] = {0,};
    rpy2mat(des_pose[5], des_pose[4], des_pose[3], mat);
    mat2zyx(mat, ori_d);
    //    ori_d[0] = des_pose[3];
    //    ori_d[1] = des_pose[4];
    //    ori_d[2] = des_pose[5];
...
//        body0 = &body[indx - 1];
        ...
        mat(body[indx].rict_dot, body[indx].wi, 3, 3, 3, rict_dot_wi);
        for (uint i = 0; i < 3; i++){
            body[indx].Qih_g[i] = body[indx].fic[i];
            body[indx].Qih_c[i] = body[indx].mi*rict_dot_wi[i];
            body[indx].Qih[i] = body[indx].Qih_g[i] + body[indx].Qih_c[i];
        }
        double rict_fic[3] = {0,}, rict_rict_dot_wi[3] = {0,}, Jic_wi[3] = {0,}, wit_Jic_wi[3] = {0,};
        mat(body[indx].rict, body[indx].fic, 3, 3, 3, rict_fic);
        mat(body[indx].rict, rict_dot_wi, 3, 3, 3, rict_rict_dot_wi);
        mat(body[indx].Jic, body[indx].wi, 3, 3, 3, Jic_wi);
        mat(body[indx].wit, Jic_wi, 3, 3, 3, wit_Jic_wi);
        for (uint i = 0; i < 3; i++){
            body[indx].Qih_g[i + 3] = rict_fic[i];
            body[indx].Qih_c[i + 3] = body[indx].mi*rict_rict_dot_wi[i] - wit_Jic_wi[i];
        ...
        mat(body[indx].rit_dot, body[indx].Hi, 3, 3, 3, rit_dot_Hi);
        mat(body[indx].rit, body[indx].Hi_dot, 3, 3, 3, rit_Hi_dot);
        for(uint i = 0; i < 3; i++){
            body[indx].Di[i] = (rit_dot_Hi[i] + rit_Hi_dot[i])*body[indx].qi_dot;
