...
//    // Write new baudnum
//    dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, ID, ADDR_BAUD_RATE, BR_1M, &dxl_error);
//    if (dxl_comm_result != COMM_SUCCESS)
//    {
//        printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
//    }
...
//    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_BAUD_RATE, BR_4M, &dxl_error);

    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_LED_R, 255, &dxl_error);
    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_LED_G, 0, &dxl_error);
    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_LED_B, 0, &dxl_error);
    ...
    packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PROFILE_ACCELERATION, 10, &dxl_error); // df : 0
    packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PROFILE_VELOCITY, 5, &dxl_error); // df : 0
...
//     packetHandler->write2ByteTxRx(portHandler, ID, ADDR_POSITION_P_GAIN, 800, &dxl_error); // df : 800
    ...
    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_RETURN_DELAY_TIME, 10, &dxl_error);
...
{
    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_LED_R, 0, &dxl_error);
    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_LED_G, 0, &dxl_error);
    packetHandler->write1ByteTxRx(portHandler, ID, ADDR_LED_B, 0, &dxl_error);
    // Reset home position
    int32_t pos = 0;
