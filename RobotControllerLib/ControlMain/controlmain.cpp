            ...
            if (dataControl->PathData.path_data_indx >= dataControl->PathData.movePath[dataControl->PathData.path_struct_indx].data_size - 1){
                if (delay_cnt >= delay_cnt_max){
                    if (fork_cnt_max > 0){
                        fork_cnt++;
                        if (fork_cnt <= 1000){
                            if(fabs(static_cast<int32_t>((-90)*dataControl->DEG2ENC) + dataControl->RobotData.offset[5] - dataControl->RobotData.present_joint_position[5]) < 2){
                                module->setGoalPosition(5, dataControl->RobotData.present_joint_position[5] - 30);
                            }
                        }
                        else if (fork_cnt >= 2000 && fork_cnt <= 3000){
                            if(fabs(dataControl->RobotData.command_joint_position[5] - dataControl->RobotData.present_joint_position[5]) < 2){
                                module->setGoalPosition(5, dataControl->RobotData.present_joint_position[5] + 30);
                            }
                        }
                    }
